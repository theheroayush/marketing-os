...
(function(){
  ...
  // Cache parsed JSON objects in memory to reduce JSON.parse overhead
  // on repeated synchronous localStorage reads.
  let _cachedSessions = null;
  let _cachedProfiles = null;

  const Storage = {
    getSessions: () => {
      if (!_cachedSessions) _cachedSessions = JSON.parse(localStorage.getItem('marku_sessions') || '[]');
      return _cachedSessions.map(s => ({ ...s, messages: s.messages ? [...s.messages] : [] }));
    },
    saveSession: (session) => {
      ...
      _cachedSessions = sessions.slice(0, 50);
      localStorage.setItem('marku_sessions', JSON.stringify(_cachedSessions));
    },
    deleteSession: (id) => {
      _cachedSessions = Storage.getSessions().filter(s => s.id !== id);
      localStorage.setItem('marku_sessions', JSON.stringify(_cachedSessions));
      if (currentView === 'history') app.renderHistoryView();
    },
    getProfiles: () => {
      if (!_cachedProfiles) _cachedProfiles = JSON.parse(localStorage.getItem('marku_profiles') || '[{"id":"default","name":"Default Profile","content":"","team":[]}]');
      return _cachedProfiles.map(p => ({ ...p, team: p.team ? [...p.team] : [] }));
    },
