...
<div class="h-full w-1/2 bg-gradient-to-r from-tertiary to-primary"></div>
  ...
  function renderSkillsHub() {
    ...
    const isAllCats = catFilter === 'All';
    const filtered = window.SKILLS.filter(s => {
      // ⚡ Bolt: Early return for category mismatch avoids expensive string allocations and .includes() checks
      if (!isAllCats && s.cat !== catFilter) return false;
      if (!q) return true;
