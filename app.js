...
(function(){
  ...
  // ⚡ Bolt: Added in-memory caching to prevent expensive JSON.parse calls on every render
  const Storage = {
    _cache: { sessions: null, profiles: null },
    getSessions: () => {
    ...
    saveSession: (session) => {
      ...
      Storage._cache.sessions = sessions.slice(0, 50);
      localStorage.setItem('marku_sessions', JSON.stringify(Storage._cache.sessions));
    },
    deleteSession: (id) => {
      Storage._cache.sessions = Storage.getSessions().filter(s => s.id !== id);
      localStorage.setItem('marku_sessions', JSON.stringify(Storage._cache.sessions));
