    ...
    container.innerHTML = `
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <button aria-label="Close modal" class="modal-close" onclick="app.closeModal()">
      ...
      body: `
        ...
        <div class="form-group">
          ...
          <div style="display:flex; gap:8px;">
            ...
            <button aria-label="Add team member" onclick="app.addTeamMemberToModal()" class="btn btn-primary" style="padding:0 12px;"><span class="material-symbols-outlined">add</span></button>
          </div>
  ...
  app.renderModalTeamList = function() {
    ...
    list.innerHTML = window.modalTeam.map((email, idx) => `
      <div class="team-member">
        ...
        <button aria-label="Remove team member" onclick="app.removeFromModalTeam(${idx})" class="remove-member">
  ...
  function renderChatView() {
    ...
    el.innerHTML = `
      <div class="skill-header" style="position:sticky;top:60px;background:var(--bg);z-index:10;padding-bottom:10px;border-bottom:1px solid ${accentColor}44;margin-bottom:16px;">
        <button aria-label="Go back to history" class="back-btn" onclick="app.navigate('history')"><span class="material-symbols-outlined">history</span></button>
        ...
        <div style="display:flex;gap:8px;">
          <button aria-label="Export PDF" class="icon-btn" onclick="app.exportPDF()" title="Export Customized PDF"><span class="material-symbols-outlined">picture_as_pdf</span></button>
          <button aria-label="Restart Session" class="icon-btn" onclick="app.openSkill('${activeSkill.id}')" title="New Session" style="color:var(--text-dim)"><span class="material-symbols-outlined">restart_alt</span></button>
          <button aria-label="Setup API Key" class="icon-btn" onclick="app.triggerSetupApiKey()" title="Setup API Key" style="color:var(--text-dim)"><span class="material-symbols-outlined">key</span></button>
        </div>
      </div>
      ...
      <div class="chat-input-wrapper">
        <form onsubmit="app.sendChatMessage(event)" style="position:relative;max-width:600px;margin:0 auto;display:flex;gap:8px;align-items:flex-end;">
          ...
          <button aria-label="Send message" type="submit" disabled style="position:absolute;right:8px;bottom:6px;width:32px;height:32px;border-radius:8px;border:none;background:${accentColor};color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;opacity:0.5;transition:0.2s;" id="chat-send-btn">
