    ...
    container.innerHTML = `
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <button class="modal-close" aria-label="Close modal" title="Close" onclick="app.closeModal()">
      ...
      body: `
        ...
        <div class="form-group">
          ...
          <div style="display:flex; gap:8px;">
            ...
            <button onclick="app.addTeamMemberToModal()" class="btn btn-primary" aria-label="Add team member" title="Add member" style="padding:0 12px;"><span class="material-symbols-outlined">add</span></button>
          </div>
  ...
  app.renderModalTeamList = function() {
    ...
    list.innerHTML = window.modalTeam.map((email, idx) => `
      <div class="team-member">
        ...
        <button onclick="app.removeFromModalTeam(${idx})" class="remove-member" aria-label="Remove team member" title="Remove member">
  ...
  function renderSkillsHub() {
    ...
    el.innerHTML = `
      ...
      <div style="background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:10px 14px;display:flex;align-items:center;gap:8px;margin-bottom:16px;">
        ...
        ${searchQ ? `<button onclick="app.clearSearch()" aria-label="Clear search" title="Clear search" style="background:none;border:none;color:var(--text-muted);cursor:pointer;font-size:18px;">&times;</button>` : ''}
      </div>
  ...
  function renderChatView() {
    ...
    el.innerHTML = `
      <div class="skill-header" style="position:sticky;top:60px;background:var(--bg);z-index:10;padding-bottom:10px;border-bottom:1px solid ${accentColor}44;margin-bottom:16px;">
        <button class="back-btn" aria-label="Back to history" title="Back to history" onclick="app.navigate('history')"><span class="material-symbols-outlined">history</span></button>
        ...
        <div style="display:flex;gap:8px;">
          <button class="icon-btn" onclick="app.exportPDF()" aria-label="Export Customized PDF" title="Export Customized PDF"><span class="material-symbols-outlined">picture_as_pdf</span></button>
          <button class="icon-btn" onclick="app.openSkill('${activeSkill.id}')" aria-label="New Session" title="New Session" style="color:var(--text-dim)"><span class="material-symbols-outlined">restart_alt</span></button>
      ...
      <div class="chat-input-wrapper">
        <form onsubmit="app.sendChatMessage(event)" style="position:relative;max-width:600px;margin:0 auto;display:flex;gap:8px;align-items:flex-end;">
          ...
          <button type="submit" disabled aria-label="Send message" title="Send message" style="position:absolute;right:8px;bottom:6px;width:32px;height:32px;border-radius:8px;border:none;background:${accentColor};color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;opacity:0.5;transition:0.2s;" id="chat-send-btn">
