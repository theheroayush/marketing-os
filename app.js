...
(function(){
  ...
  const Storage = {
    getSessions: () => {
      if (_cachedSessions) return _cachedSessions.map(s => ({ ...s }));
      _cachedSessions = JSON.parse(localStorage.getItem('marku_sessions') || '[]');
      return _cachedSessions.map(s => ({ ...s }));
    },
    saveSession: (session) => {
    ...
    getProfiles: () => {
      if (_cachedProfiles) return _cachedProfiles.map(p => ({ ...p, team: p.team ? [...p.team] : [] }));
      let ps = JSON.parse(localStorage.getItem('marku_profiles') || '[{"id":"default","name":"Default Profile","content":"","team":[]}]');
      _cachedProfiles = ps.map(p => ({ ...p, team: p.team || [] }));
      return _cachedProfiles.map(p => ({ ...p, team: p.team ? [...p.team] : [] }));
    },
    saveProfiles: (profiles) => {
      _cachedProfiles = profiles.map(p => ({ ...p, team: p.team ? [...p.team] : [] }));
