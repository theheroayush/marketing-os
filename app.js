...
(function(){
  ...
  const Storage = {
    ...
    getProfiles: () => {
      ...
      return _profilesCache.map(p => ({ ...p, team: [...p.team] }));
    },
    saveProfiles: (profiles) => {
      const normalized = profiles.map(p => ({ ...p, team: p.team || [] }));
      _profilesCache = normalized;
      localStorage.setItem('marku_profiles', JSON.stringify(normalized));
    },
