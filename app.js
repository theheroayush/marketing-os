...
(function(){
  ...
  const Storage = {
    ...
    saveSession: (session) => {
      ...
      _sessionsCache = newSessions.map(s => ({ ...s, messages: s.messages ? [...s.messages] : [] }));
      localStorage.setItem('marku_sessions', JSON.stringify(newSessions));
    },
    deleteSession: (id) => {
      const sessions = Storage.getSessions().filter(s => s.id !== id);
      _sessionsCache = sessions.map(s => ({ ...s, messages: s.messages ? [...s.messages] : [] }));
    ...
    saveProfiles: (profiles) => {
      const normalizedProfiles = profiles.map(p => ({ ...p, team: p.team ? [...p.team] : [] }));
      _profilesCache = normalizedProfiles.map(p => ({ ...p, team: p.team ? [...p.team] : [] }));
      localStorage.setItem('marku_profiles', JSON.stringify(normalizedProfiles));
    },
